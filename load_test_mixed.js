...
// allow for different read/write IP for master-slave scenario
const readIp = '<read-ip>';
const writeIp = '<write-ip>';
const username = '';
const password = '';
...
export default function () {
  ...
  if (mod < 7) {
    ...
    if (rnd < 0.9) {    
      ...
      http.get(`http://${readIp}:5984/wiki_articles/${documentId.id}`, authOptions);
    } else {    
      let keyword = searchKeywords[Math.floor(Math.random()*searchKeywords.length)]; // get random keyword
      http.get(`http://${readIp}:5984/wiki_articles/_design/wiki_articles_by_keyword/_view/wiki_articles_by_keyword?key=%22${keyword}%22&limit=10&stable=false&update=lazy`, authOptions);
    }    
  }
  else {
    // write scenario
    if (rnd < 0.3) {    
      ...
      http.post(`http://${writeIp}:5984/wiki_articles`, article, authOptions);    
    } else if (rnd < 9.0) {    
      ...
      http.put(`http://${writeIp}:5984/wiki_articles/${itemToUpdate.id}`, article, authOptions);           
    } else {
      ...
      http.del(`http://${writeIp}:5984/wiki_articles/${itemToDelete.id}?rev=${itemToDelete.rev}`, null, authOptions);           
    }
  }
}
